...
export default function StatsPage() {
    ...
    const handleSelectAll = useCallback(() => setSelectedUserIds(new Set(users.map((u) => u.id))), [users]);
    ...
    const fetchFromWhenIWork = useCallback(async () => {
        ...
        if (rangeDays > 45) toast.info("This may take a few minutes to fetch from When I Work.", { duration: 6000 });
        ...
        try {
            ...
            const response = await fetch(route('admin.stats.sync'), {
                ...
                body: JSON.stringify({ user_ids: userIds, start_date: filter.start_date, end_date: filter.end_date }),
            });
    ...
    const applyCustom = useCallback(() => {
        if (!customStart || !customEnd) { toast.error('Select start and end dates'); return; }
        reloadInertia({ filter_type: 'custom', start_date: toYmd(customStart), end_date: toYmd(customEnd), per_page: perPage, page: 1 });
    }, [customEnd, customStart, perPage, reloadInertia]);

    const handleFilterTypeChange = useCallback((v: FilterType) => {
        setFilterType(v);
        if (v === 'month') reloadInertia({ filter_type: 'month', year: selectedYear, month: selectedMonth, per_page: perPage, page: 1 });
        else if (v === 'year') reloadInertia({ filter_type: 'year', year: selectedYear, per_page: perPage, page: 1 });
    }, [perPage, reloadInertia, selectedMonth, selectedYear]);

    const handleYearChange = useCallback((v: string) => {
        const y = Number(v);
        setSelectedYear(y);
        if (filterType === 'month') reloadInertia({ filter_type: 'month', year: y, month: selectedMonth, per_page: perPage, page: 1 });
        else if (filterType === 'year') reloadInertia({ filter_type: 'year', year: y, per_page: perPage, page: 1 });
    }, [filterType, perPage, reloadInertia, selectedMonth]);

    const handleMonthChange = useCallback((v: string) => {
        const m = Number(v);
        setSelectedMonth(m);
        if (filterType === 'month') reloadInertia({ filter_type: 'month', year: selectedYear, month: m, per_page: perPage, page: 1 });
    }, [filterType, perPage, reloadInertia, selectedYear]);

    const handlePerPageChange = useCallback((v: string) => {
        const pp = Number(v);
        setPerPage(pp);
        if (filterType === 'month') reloadInertia({ filter_type: 'month', year: selectedYear, month: selectedMonth, per_page: pp, page: 1 });
        else if (filterType === 'year') reloadInertia({ filter_type: 'year', year: selectedYear, per_page: pp, page: 1 });
        else reloadInertia({ filter_type: 'custom', start_date: filter.start_date, end_date: filter.end_date, per_page: pp, page: 1 });
    ...
    const goToPage = useCallback((nextPage: number) => {
        ...
        if (filterType === 'month') reloadInertia({ ...base, year: selectedYear, month: selectedMonth });
        else if (filterType === 'year') reloadInertia({ ...base, year: selectedYear });
        else reloadInertia({ ...base, start_date: filter.start_date, end_date: filter.end_date });
            ...
            <div className="container mx-auto px-4 sm:px-6 py-6 max-w-7xl">
                    ...
                    <div className="flex items-end justify-between gap-4 flex-wrap">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-gray-500">
                                Period&nbsp;
                                <span className="font-medium text-red-700 tabular-nums">{rangeLabel}</span>
                            </p>
                        </div>
                        ...
                        <Button
                            size="sm"
                            className="bg-red-600 hover:bg-red-700 active:bg-red-800 text-white shadow-sm gap-2 h-9 px-4 text-sm font-medium transition-colors cursor-pointer"
                        ...
                        >
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <path fillRule="evenodd" d="M10 10a.75.75 0 0 1 .75.75v3.69l1.22-1.22a.75.75 0 1 1 1.06 1.06l-2.5 2.5a.75.75 0 0 1-1.06 0l-2.5-2.5a.75.75 0 1 1 1.06-1.06l1.22 1.22V10.75A.75.75 0 0 1 10 10Z" clipRule="evenodd"/>
                            </svg>
                ...
                <Card className="border border-gray-200 shadow-sm rounded-xl overflow-hidden">
                        ...
                        <div className="flex items-center justify-between gap-3 flex-wrap">
                            ...
                            <div className="flex items-center gap-2.5">
                                <Collapsible open={filtersOpen} onOpenChange={setFiltersOpen}>
                                    <CollapsibleTrigger asChild>
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            className="h-8 gap-1.5 border-gray-200 text-gray-600 hover:border-gray-300 hover:text-gray-900 text-xs font-medium cursor-pointer"
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor">
                                                <path fillRule="evenodd" d="M2.628 1.601C5.028 1.206 7.49 1 10 1s4.973.206 7.372.601a.75.75 0 0 1 .628.74v2.288a2.25 2.25 0 0 1-.659 1.59l-4.682 4.683a2.25 2.25 0 0 0-.659 1.59v3.037c0 .684-.31 1.33-.844 1.757l-1.937 1.55A.75.75 0 0 1 8 18.25v-5.757a2.25 2.25 0 0 0-.659-1.591L2.659 6.22A2.25 2.25 0 0 1 2 4.629V2.34a.75.75 0 0 1 .628-.74Z" clipRule="evenodd"/>
                                            </svg>
                                            Filters
                                            <svg xmlns="http://www.w3.org/2000/svg" className={`h-3 w-3 transition-transform ${filtersOpen ? 'rotate-180' : ''}`} viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <Select value={String(perPage)} onValueChange={handlePerPageChange}>
                                    <SelectTrigger className="h-8 w-[120px] border-gray-200 text-xs text-gray-600 cursor-pointer">
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                            ...
                            <div className="flex items-center gap-2">
                                {selectionCount > 0 && (
                                    <span className="text-xs text-gray-500">
                                        <span className="font-semibold text-red-700">{selectionCount}</span> selected
                                    </span>
                                )}
                                <Button
                                    size="sm"
                                    variant="ghost"
                                    className="h-8 text-xs text-gray-600 hover:text-gray-900 hover:bg-gray-50 px-3 cursor-pointer"
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                                        ...
                                        <div className="space-y-1.5 lg:col-span-2">
                                            ...
                                            <Select value={filterType} onValueChange={(v) => handleFilterTypeChange(v as FilterType)}>
                                                <SelectTrigger className="h-9 bg-white border-gray-200 text-sm cursor-pointer">
                                        ...
                                        {filterType === 'custom' && (
                                            <>
                                                ...
                                                <div className="flex items-end">
                                                    <Button
                                                        size="sm"
                                                        className="w-full h-9 bg-gray-900 hover:bg-gray-800 text-white text-sm font-medium cursor-pointer"
                                                        onClick={applyCustom}
                                                        disabled={isLoading}
                                                    >
                                                        Apply range
                    ...
                    <CardContent className="p-0">
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow className="bg-gray-50/80 hover:bg-gray-50/80 border-b border-gray-100">
                                        <TableHead className="w-12 pl-5 pr-3">
                                            <Checkbox
                                                checked={allSelected ? true : someSelected ? 'indeterminate' : false}
                                                onCheckedChange={(v) => { if (Boolean(v)) handleSelectAll(); else handleClearAll(); }}
                                    ...
                                    {isLoading ? (
                                        ...
                                        Array.from({ length: 5 }).map((_, i) => (
                                            <TableRow key={i} className="border-b border-gray-50">
                                                ...
                                                {[...Array(5)].map((_, j) => (
                                                    <TableCell key={j}>
                                                        <div className="h-3.5 w-12 rounded bg-gray-100 animate-pulse ml-auto" />
                                                    </TableCell>
                                                ))}
                                            </TableRow>
                                        ))
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <TableCell className="py-3.5">
                                                        <div className="flex items-center gap-2.5">
                                                            {/* Avatar placeholder */}
                                                            <div className="flex-shrink-0 h-8 w-8 rounded-full bg-red-100 flex items-center justify-center text-xs font-semibold text-red-700 select-none">
                                                            ...
                                                            <div className="min-w-0">
                                                                <div className="flex items-center gap-1.5 flex-wrap">
                                                                    ...
                                                                    {u.priority != null && (
                                                                        <span className="inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-semibold ring-1 ring-inset ring-red-200/60 bg-red-50 text-red-700">
                                                                            P{u.priority}
                                                                        </span>
                                                                    )}
                                                                </div>
                                                    ...
                                                    <TableCell className="text-right">
                                                        <span className="text-sm font-semibold text-gray-800 tabular-nums">
                                                            {r ? r.total_duty_days : <span className="text-gray-300 font-normal">—</span>}
                                                        </span>
                                                    </TableCell>
                                                    <TableCell className="text-right">
                                                        <span className="text-sm font-semibold text-gray-800 tabular-nums">
                                                            {r ? r.leave_taken : <span className="text-gray-300 font-normal">—</span>}
                                                        </span>
                                                    </TableCell>
                                                    <TableCell className="text-right">
                                                        <span className="text-sm font-semibold text-gray-800 tabular-nums">
                                                            {r ? r.upcoming_leave : <span className="text-gray-300 font-normal">—</span>}
                                                        </span>
                                                    </TableCell>
                        ...
                        {/* ── Pagination ── */}
                        <div className="flex items-center justify-between gap-4 px-5 py-3.5 border-t border-gray-100 bg-gray-50/40 flex-wrap">
                            ...
                            <div className="flex items-center gap-1.5">
                                <Button
                                    variant="outline"
                                    size="sm"
                                    className="h-7 px-3 text-xs border-gray-200 text-gray-600 hover:border-gray-300 disabled:opacity-40"
                                ...
                                <div className="flex items-center gap-1 px-2">
                                    <span className="text-xs font-semibold text-gray-700">{pagination.current_page}</span>
                                    <span className="text-xs text-gray-300">/</span>
                                    <span className="text-xs text-gray-400">{pagination.last_page}</span>
                                </div>
